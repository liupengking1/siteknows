...
public class OnlineDBAdapter {
	...
	private static final String DATABASE_NAME = "map_calibrate";
	...
	public static final String KEY_POINTS_INDEX = "positionIndex";
	...
	private static final int DATABASE_VERSION = 2; // Added a comment column in
													// the maps table

	private static final String DATABASE_CREATE_TABLE_1 = "create table maps ("
			+ KEY_MAP_ROWID + " integer primary key autoincrement,"
			+ KEY_MAP_FILENAME + " text not null," + KEY_MAP_FILEPATH
			+ " text not null," + KEY_MAP_COMMENT + " text);";
	...
	private static final String DATABASE_CREATE_TABLE_2 = "create table referencepoints ("
			...
			+ KEY_POINTS_INDEX
			+ " integer,"
			+ KEY_POINTS_MAPX + " real," + KEY_POINTS_MAPY + " real);";

	private DatabaseHelper DBHelper;
	private SQLiteDatabase db;

	public OnlineDBAdapter(Context context) {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (oldVersion == 1 && newVersion == 2) {
				db.execSQL("ALTER TABLE " + DATABASE_TABLE_1 + " ADD COLUMN "
						+ KEY_MAP_COMMENT + " TEXT;");
				// db.execSQL("ALTER TABLE " + DATABASE_TABLE_1 +
				// " DROP COLUMN " + KEY_MAP_COMMENT);
			}
		}
	}

	// ---opens the database---
	public OnlineDBAdapter open() throws SQLException {
	 ...
	 * @return
	 */
	private long insertReferencePoint(long mapkey, int position, float mapX,
			float mapY) {
		ContentValues initialValues = new ContentValues();
		initialValues.put(KEY_POINTS_MAPKEY, mapkey);
		initialValues.put(KEY_POINTS_INDEX, position);
	...
	public long insertReferencePoint(long mapkey, CoordinateMapping point) {
		return insertReferencePoint(mapkey, point.WifiCoordinateIndex,
				point.mapCoordinate.x, point.mapCoordinate.y);
	}
	...
	public boolean updateMapComment(long mapKey, String comment) {
		...
		int affectedRows = db.update(DATABASE_TABLE_1, updateValues,
	...
	public String getMapComment(long mapKey) throws SQLException {
		...
		Cursor mCursor = db.query(DATABASE_TABLE_1,
				new String[] { KEY_MAP_COMMENT }, selection, selectionArgs,
				null, null, null);
		String mapComment = "";
		if (mCursor != null) {
			if (mCursor.moveToFirst()) {
				int columnIndex = mCursor
						.getColumnIndexOrThrow(KEY_MAP_COMMENT);
	...
	public MapData getMapData(long mapKey) throws SQLException {
		String selection = KEY_MAP_ROWID + " =? ";
		String[] selectionArgs = new String[] { Long.toString(mapKey) };
		Cursor mCursor = db.query(DATABASE_TABLE_1, new String[] {
				KEY_MAP_FILENAME, KEY_MAP_FILEPATH, KEY_MAP_COMMENT },
				selection, selectionArgs, null, null, null);

		MapData mapData = new MapData();
		if (mCursor != null) {
			if (mCursor.moveToFirst()) {
				mapData.fileName = mCursor.getString(mCursor
						.getColumnIndexOrThrow(KEY_MAP_FILENAME));
				mapData.filePath = mCursor.getString(mCursor
						.getColumnIndexOrThrow(KEY_MAP_FILEPATH));
				mapData.comment = mCursor.getString(mCursor
						.getColumnIndexOrThrow(KEY_MAP_COMMENT));
				mCursor.close();
			}
		}
	...
	public ArrayList<CoordinateMapping> getAllReferencePointsForMap(long mapkey) {
		Cursor referencePointsCursor = db.query(DATABASE_TABLE_2, new String[] {
				KEY_POINTS_INDEX, KEY_POINTS_MAPX, KEY_POINTS_MAPY },
				KEY_POINTS_MAPKEY + "=" + mapkey, null, null, null, null, null);
		...
		if (referencePointsCursor != null) {
			...
			while (rowsLeft) {
				referencePoints.add(new CoordinateMapping(new PointF(
						...
						referencePointsCursor.getFloat(referencePointsCursor
								.getColumnIndex(KEY_POINTS_MAPY))),
						referencePointsCursor.getInt(referencePointsCursor
								.getColumnIndex(KEY_POINTS_INDEX))));
	...
	public Cursor getAllReferencePoints(long mapkey) {
		// Tables need to have a column called _id for CursorAdapter to work.
		// Therefore I need
		// a raw sql call instead of the convenience functions.

		String query = "SELECT " + KEY_POINTS_ROWID + " as _id, "
				+ KEY_POINTS_INDEX + ", " + KEY_POINTS_MAPX + ", "
				+ KEY_POINTS_MAPY + " FROM " + DATABASE_TABLE_2 + " WHERE "
				+ KEY_POINTS_MAPKEY + "=" + mapkey;
	...
	public Cursor getAllMaps() {
		...
		String query = "SELECT " + KEY_MAP_ROWID + " as _id, "
				+ KEY_MAP_FILENAME + ", " + KEY_MAP_FILEPATH + ", "
				+ KEY_MAP_COMMENT + ", " + "(SELECT COUNT(" + KEY_POINTS_MAPKEY
				+ ") FROM " + DATABASE_TABLE_2 + " WHERE " + KEY_POINTS_MAPKEY
				+ "=" + KEY_MAP_ROWID + ") as " + KEY_NBR_OF_POINTS + " FROM "
				+ DATABASE_TABLE_1;
	...
	public String getMapPath(long mapkey) throws SQLException {
		...
		Cursor mCursor = db.query(DATABASE_TABLE_1,
				new String[] { KEY_MAP_FILEPATH }, where, null, null, null,
				null);
		String mapPath = "";
		if (mCursor != null) {
			if (mCursor.moveToFirst()) {
